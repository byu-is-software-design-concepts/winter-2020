...
describe("moveZeroesToEnd", () => {
  ...
  it("should move one zero to the end with an array of numbers", () => {
    const result = moveZeroesToEnd([0, 2, 1, 7]);
    const expected = [2, 1, 7, 0];

    expect(result).toEqual(expected);
  });
  it("should move all zeros to end", () => {
    const result = moveZeroesToEnd([3, 0, 0, 1]);
    const expected = [3, 1, 0, 0];

    expect(result).toEqual(expected);
  });
  it("should move one zero to the end with an array of various types", () => {
    const result = moveZeroesToEnd([true, 0, 2, false, "1"]);
    const expected = [true, 2, false, "1", 0];

    expect(result).toEqual(expected);
  });
  it("should move all zeros to end", () => {
    const result = moveZeroesToEnd([0, 0, 0, 0, 0]);
